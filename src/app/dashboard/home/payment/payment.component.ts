...
export class PaymentComponent implements OnInit, AfterViewInit {
  ...
  transfer(): void {
    ...
    if (this.to.toLowerCase() === 'cloudcoin.bitmart.com')
    {
      this.showErrorMessage('Payment cannot be used to send money to this Merchant Address');
      return;
    }
    if (!this.skywallet || this.skywallet === '' )
    {
      this.showErrorMessage('Please enter your SkyWallet address as sender');
      return;
    }
    ...
    if (!this.auth.getLoggedIn())
    {
      ...
      const params = {an: coin.an, amount: this.amount, to: this.to, memo : this.memo, sender_name: this.skywallet };
      this.showLoading(true);
