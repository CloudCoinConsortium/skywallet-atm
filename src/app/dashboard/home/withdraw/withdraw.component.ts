...
export class WithdrawComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('download_coin') && localStorage.getItem('amount'))
    {
      this.localCoin = true;
    }
  }
  withdraw(): void {
    ...
    if (!this.auth.getLoggedIn())
    {
      ...
      this.raida.receive(params).then(response => {
       // console.log(response);
        ...
        localStorage.setItem('download_coin', cstr);
    ...
    reDownload(): void {
      const coinstack = localStorage.getItem('download_coin');
      const amount = localStorage.getItem('amount');
      const filename = amount + '.CloudCoin.' + Date.now() + '.stack';
      this.download(filename, coinstack);
